...
class DDPG_env2():
    ...
    def map_state(self, state):
        ...
        return (np.array([tyre_map[tyre], condition, fuel_level, cur_weather_map[cur_weather], radius, laps_cleared, fuel_capacity]))
    ...
    def save(self):
        ...
        """
        torch.save(self.actor.state_dict(), self.directory + 'DDPG_env2_actor.pth')
        torch.save(self.critic.state_dict(), self.directory + 'DDPG_env2_critic.pth')

    def load(self):
        """
        Loads the state dictionaries of the actor and critic networks to files
        """
        self.actor.load_state_dict(torch.load(self.directory + 'DDPG_env2_actor.pth'))
        self.critic.load_state_dict(torch.load(self.directory + 'DDPG_env2_critic.pth'))
...
class Critic(nn.Module):
    ...
    def forward(self, state, action):
        return self.net(torch.cat((state.reshape(1,7), action.reshape(1,1)),1))
